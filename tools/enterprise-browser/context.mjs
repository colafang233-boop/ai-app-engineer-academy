...
export async function createEnterpriseBrowserContext() {
  ...
  for (let attempt = 0; attempt < 50; attempt += 1) {
    ...
    if (attempt === 49) throw new Error('Enterprise browser static server did not start');
  ...
  async function enterLesson(number) {
    ...
    assert.equal(await page.locator('[data-enterprise-context]').count(), 1);
    assert.equal(await page.locator('[data-enterprise-knowledge]').count(), 1);
    assert.ok(await page.locator('[data-enterprise-knowledge] .enterprise-glossary article').count() >= 4);
    assert.ok(await page.locator('.cr-official-reference a').count() >= 3);
  ...
  async function runDecision(answers) {
    for (let index = 0; index < answers.length; index += 1) {
      ...
      if (index < answers.length - 1) await page.waitForTimeout(100);
    }
